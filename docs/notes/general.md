...
- `継承`:
  - `主な手法`:
    - `インターフェース継承`：`実行時多相性（run-time polymorphism）`, `動的多相性（dynamic polymorphism）`と呼ばれる
      ...
      - オブジェクトのアクセスにポインタや参照経由で直接アクセスするときは、コンパイラは型がわかっているため`実行時多相性（run-time polymorphism）`は不要である(virtual を付けずに単に規定クラスの関数をオーバーライドするだけで良い)
  ...
  - `オーバーライド制限`:
    - `virtual`: 派生クラス先でオーバーライド可能である
    - `=0`: 関数は virtual であって、派生クラス先で必ずオーバーライドしなければならない
    - `override`: 派生クラス先で基底クラスの仮想関数をオーバーライド 
    - `final`: 派生クラス先でオーバーライドすることができない