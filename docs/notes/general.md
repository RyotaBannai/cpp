...
- `数値演算`:
  - `epsilon` は浮動小数点数で `1+epsilon-1` が 0 よりも大きくなる生の最小値を表す(1163)
  - エラーは `<cerrno>` が定義する `errno`への代入によって通知される. 定義域エラーなら`EDOM`であり、値域エラーなら`ERANGE` である.(1165)
  - 標準ライブラリの complex は縮小変換に対する保護を行わないため、意図しないのであれば `{}` 構文を利用(1167)
  - `valarray`:
    - 添字演算: 引数が `const` であれば、`要素のコピー`が演算結果となり、`非 const` であれば、`要素への参照`が演算結果となる.(1170)
      - C++ は参照の配列を直接サポートしていないため(`valarray<int&>` のような型)、擬似的な実装となっている.
...
- 並列処理:
  - thread や mutex を直接利用するのではなく、packaged_task と future を優先する(1210)
  - 単純なカウンタには、atomic そうでなければ、mutex や condition_variable を使う
  - アトミックを使うとロックフリープログラミングが可能になる. (ただロックフリープログラミングは高度なので一般のプログラマはロックを用いる方が良い)
  - `メモリモデル(memory model)`: メモリへの並行的なアクセス、通常期待されるような単純で平凡なアクセうと同じように行えること
    - IOS C++ 標準では、`大半のプログラマが現代のコンピュータハードウェアの詳細を意識しないで済むことを保証するための、システム実装者とプログラマとの契約である`と定義している.(1195)
    - メモリ上のオブジェクトに対する処理:
      - メモリ上のオブジェクトに対する処理は、メモリ上のそのオブジェクトを直接操作しているわけではなく、オブジェクトがレジスタにロードされ、そこで変更され、その後でメモリに書き戻される.
      - さらに言うと、オブジェクトは、まずメインメモリからキャッシュメモリへとロードされて、キャッシュメモリからレジスタへとロードされるのが一般的である.
    - `メモリロケーション`:
      - マシンが１ワードよりも小さな単位ではロードもストアもできない場合は、１ワードに異なる変数を格納していてかつ別々のスレッドが使用する状況下でデータの不整合が起こる(1196)
      - このような混沌をメモリモデルでは`メモリロケーション`と言う手法を用いて解決している
        - この動作を保証するのはコンパイラにかかっている
    - `メモリオーダリング(memory ordering)`:
      - スレッドがメモリ上の値を参照する際に見えるものに対してプログラマが想定できることを表す用語である.(1198)
      - もっとも単純なメモリオーダは、`逐次一貫(sequentially consistent)`と呼ばれるもので、全てのスレッドは実行された全ての処理の結果が同じ順序として見える. その順序は、単一スレットで命令を逐次実行した場合の順序である
      - スレッドが命令の実行順序を変更することはあり得るが、他のスレッドが変更を参照する時点では、それまで実行された処理、及び（その結果である）メモリロケーションの値が的確に定義された状態だることが保証されて、全スレッドから同じものが見える.
      - メモリロケーションに一貫した観点を強制した上で値を'見る'ことを、`アトミック処理(atomic operation)` と呼ぶ
      - アトミック性:
        - `同期処理`とは、あるスレッドが他のスレッドの処理結果をいつ参照するかを判断する機能である.すなわち、ある処理の前に、完了しておくべき処理を決定する(1201)
          - 一個以上のメモリロケーションに対する同期処理は、`消費処理`、`獲得処理`、`解放処理`、`獲得・解放処理`である(1201)
        - is_lock_free: 処理がロックフリーかロックを用いて実装されているかを確認できる(1204)
          - 全ての主要な処理系では、藩整数型をポインタ型に対する is_lock_free は true を返す.
          - atomic 機能は単純な組み込み型にマッピングされることを想定しているが、`atomic<T>` の T 型が大規模であれば、ロックを用いて実装される.また、T はトリビアルにコピーできないといけない(ユーザ定義のコピー演算を持ってはいけない).
        - compare_exchange_weak: 更新対象の値を load した時点の値(expected)と比較し、同じであれば更新し、異なれば今回分の値を取得し、expected として次の比較に用いる. 常に比較結果が異なれば expected を更新するため無限ループになることはない.(1206)
          - 比較結果が異なるのは他のスレッドが更新対象の値をすでに更新した場合である.
        - compare_exchange_strong のような処理は、一般的に`比較と交換(compare-and-swap)処理`（`CAS 処理`）と呼ばれる.
          - 全ての CAS 処理には、`ABA 問題`(`the ABA problem`)と言う重大な問題が潜んでいる
            - 例えば、ロックフリーの単方向連結リスト. スレッド X が load したリスト L の先頭が A で、その先頭を更新しようとした時('レディ'であると言う)に、スレッド Y がリスト L を変更して、 先頭を B にしたとする. この時、スレッド Z がスレッド X よりも先にリスト L を更新しようとして、load 時の先頭 B と比較して同じであるためリスト L の先頭を A にしたとする. この時、スレッド X が load した時の A と、スレッド Z の更新後の A は別のものであるが問題なく更新されてしまうと言うことが起こる(1206)
        - `二重チェックロック(double-checked locking)`: x の初期化をロック下で行っておけば、x の初期化が完了した後は、x にアクセスするたびにロックを獲得するコストが節約できる、と言うもの. (つまり、x を初期化するためのロックを使うのではなく、x の初期化を一回だけ行うことを保証するためにロックを使う).(1207)
          - ここで x_init を atomic にすることで、x_init 判定と x の初期化処理がオプティマイズされず実装通りに実行されることを保証する(1207)
          - この二重ロックをイディオムを `once_flag`, `once_call` とで実装されているため、直接記述する必要はない.(1208)
        - `スピンロック（Spinlock）`: スレッドがロックを獲得できるまで単純にループ（スピン）して定期的にロックをチェックしながら待つ方式.
          - スレッドはその間有益な仕事を何もせずに動作し続けるため、これは一種の`ビジーウェイト状態`を発生させる.
          - 獲得されたスピンロックは明示的に解放するまでそのまま確保されるが、実装によってはスレッドがブロック（スリープ）したときに自動的に解放される場合もある.
          - `atomic_flag flag = ATOMIC_FLAG_INIT; void lock() { while(flag.test_and_set()); }`(1208)
        - `フェンス`: `メモリバリア(memory barrier)` として知られる.
          - ある種のメモリオーダによる命令の並び替えを制限するための機能.
          - プログラムの時効速度を安全な速度にまで落として、メモリ階層を妥当な状態に維持するのもを考えれば良い(1210)
  - `ロック不要プログラミング(programming without lock)`: データの競合を回避するための、きめ細かな低レベルの仕組み
  - スレッド: システムレベルでの並行プログラミングをサポートするコンポネント
    - タスク: タスレベルでの並行プログラミングをサポートするコンポネント
- スレッドとタスク:
  - スタックは thread 間で共有されない. (1214)
  - thread はシステム資源であるシステムスレッド(system thread) を表現する. そのため thread はムーブ可能だがコピーはできない.
  - `thread::hardware_concurrency()` はハードウェア支援によって同時に実行可能なタスク数を返却する.通常であれはコア数と同等で、`ハイパースレッディング`有効になっている場合は、コア数の２倍の数が返却される.
    - [ハイパースレッディング](https://www.intel.co.jp/content/www/jp/ja/gaming/resources/hyper-threading.html)
  - 次の 4 つのうちいずれかが成立すれば、thread の id は `id{}` となる:
    - タスクが割り当てられていない
    - 実行が終了している
    - ムーブされた
    - `detach()` された (`t.detach()`: t が表現するシステムスレッドが存在しないことを保証する)
      - 選択肢があるならば管理コストが高いため detach() を使用しない方が良い(1220)
      - どうしても detach() しなければならない場合は、スコープないの変数を参照していないことを必ず確認する(1220)
        - `int var; thread disaster{[&] { this_thread::sleep_for(second{7.3}); ++var }};`
        - 変数 var がさすメモリは再利用に利用されるが、そのデータが thread の var が参照しているため、別のデータが破壊される可能性がある: `ハイゼンバグ(Heisenbug)` と呼ばれるバグ(1220)
          - ラムダの参照キャプチャによって簡単にスコープの外に参照を持ち出すことができ、かつ、detach によって、thread もスコープの外に持ち出すことできるために起きる(1221)
  - cout は広域的な共有オブジェクトなので、２個以上の thread が同時に cout へ出力を行わないようにしない限り、識別可能な状態で文字が出力される保証はない(1216)
  - thread のコンストラクタに参照を渡す際には、参照ラッパ(968)を用いる必要がある(1217)
    - `void test(vector<double> &v){ thread my_t1 {my_task, v}; // v はコピー thread my_t2 {my_task, ref(v)}; // v は参照 thread my_t13 {[&v]{ my_task(v); }}; // 参照キャプチャにより ref() 回避 }`
  - スレッドの解体:
    - システムスレッドを謝って消さないようにするために、thread が `joinable()` (`get_id() != id{}`)であれば、thread デストラクタは、プログラムを終了させるために`terminate()`を呼び出す
    - thread のデストラクタは join() を実行しないため、解体されても thread がその後も生存し続ける. そのためえ guarded_thread のような仕組みを作っておく必要がある(1219)
      - システムスレッドは、"永遠に存在し続ける"、あるいはいつ終了するかを地震で判断するものとして長く利用されてきたため join() しない. この種のスレッド（プロセス）は`デーモン(daemon)`と呼ばれる.
  - yield: 別の thread の処理を進める機会を与える. 別のスレッドはブロックされないため、別の thread が現在のスレッドを明示的にウェイクアップさせなくてもいつかは実行を進めらる.
    - `atomic な状態`が変化するのを待つ場合、や
    - `強調動作するマルチスレッドを待つ場合`は、yield が基本的な機能として欠かせなくなる.
  - 通常は、yield よりも `sleep_for` を使う方が良い.
    - sleep_for に与えた引数によって、`どの thread を実行するかを合理的に選択する機会をスケジューラに与える`ことができるため(1221)
    - そのため yield は極めて稀で特殊な場合に実行する最適化のための機能と覚えておくと良い.
  - thread は`プリエンプティブ`である: 全ての thread が理にかなった割合で実行されるように処理系が適切にタスクを切り替えてくれる(1221)
  - プログラマは基本的にシステムクロックを操作するべきではない:
    - `thread::wait_until` や `timed_mutex::wait_until` に影響を与える(`thread::wait_for` や `timed_mutex::wait_for` は問題ない)(1222)
  - 標準に thread のキャンセル機能はないが、次の２つの方法でプログラマが実装可能:
    - なんからのリクエストを処理するループする thread を作成し、外から`自ら穏やかに終了`と言うメッセージを送信する
    - `まだ必要とされている`ことを表す変数を定期的に調べるようような処理を追加する
  - `thread_local`:
    - 初めて利用される直前に初期化される.
    - thread_local (`スレッド記憶域期間（thread storage duratin）`を持つ)が構築されたら thread 終了時に解体される.
    - メリット:
      - 排他的にアクセスするデータ用のキャッシュを明示的に保持する thread での利用
        - キャッシュを共有するマシンでは、性能が向上する(1223)
        - データ転送を大きな単位でバッチ処理すれば、単純かやロックコストの低下につながる(1223)
    - 一般に、非局所メモリ(static)は並行プログラミングで問題となる. つまり複数の thread から同じデータを共有するためである. ここで thread_local を使用すると、同じクラスの非局所メモリは thread ごとに別々となり、競合は発生しない. ただ前ユーザが共有する単一の default_value が失われることになる.
      - thread_local と宣言できるもの: `名前空間の変数`、`局所 static`,` クラス static メンバ`(1224)
      - thread_local の実行順序は定義されないため、異なる thread_local が構築順序に依存しないようにして、可能であれば、コンパイル時あるいはリンク時の初期化を利用すると良い.
      - thread_local は static 変数と同様にデフォルトでゼロに初期化される
  - `条件変数(condition variable)`: 条件変数は、他の thread が生成したイベントやった今イベントを待つために利用する変数である(通信の管理). (1224)
    - 条件変数でデータの競合は防げない. むしろデータ競合の原因となり得るデータ共有をしないで済ませるためのものである.
    - thread は指定した時間になった、あるいは他の thrad の処理が完了した、などの何らかのイベントが発生するまで、condition_variable に基づいて待機(ブロック)することができる(1235)
    - condition_variable を解体する際には、待機している全 thread に通知する必要がある. そうしないと thread は永遠に待機するとこになる(1236)
    - condition_variable の unique_lock は unique_lock で待機している thread の衝突が原因で、thread がウェイクアップする機会を逃さないようにするため、wait 関数によって利用される(1236)
      - 単なる wait （`condition_variable.wait()` ではない `wait()`）は特に注意すべきな低レベルな処理であり、通常は高度な抽象化を実現する実装の中で利用するものである.
        - これは擬似的に thread をウェイクアップさせることもある. すなわち、他の thread が通知していないにも関わらずシステムが wait() の thread の実行を再開すると判断することがある.
        - condition_variable を使うことで consumer の処理は何度もウェイクアップして、キューに仕事があるかを調べ、空の際にどうするかの判断が必要がある(1239)
        - get() で得た値を返却値としてではなく、参照と戻すのは、例外送出の可能性があるコピーコンストラクタを持つ要素型で問題が生じないようにするためである(1238)
  - `スタベイション(starvation)`: 複数の thread がブロックされている時、ブロック解除すべき thread を選択するするのは、原則としてシステムスケジューラであり、いつまでも選択されない thread が発生してしまうこと. (1225)
    - 全ての thread に実行する機会を均等に与えるスケジューリングアルゴリズムは`公平(fair)`であると言う.
    - cout はデータ破壊から保護されることは保証されているが、異なるスレッドからの出力が混ざってしまうことに対する保護はない.(1225)
  - `クリティカルセクション(critical section)`: ロックによって保護された部分のコード
  - 標準ライブラリの mutex は排他的所有権背マンティクス(exclusive ownership semntics) を提供する:
    - つまり、ある単一の thread がその資源へのアクセス独占する.
    - 他にも、多重読み取り単一書き込みのミューテックスなどもある(1226)
    - mutex を thread に渡すときは ref を使用. これは可変個引数テンプレートを介して参照を渡すためであり、参照を渡すのはコピーもムーブもできないためである.(1233)
  - システム資源は、共有、リーク、コピー、ムーブができないので、擬似的に別個のものとして見るのが良い.
    - ロックは一種の資源なので、必ず解放しなければならない. すなわち、m.lock() には対応する m.unlock() が必要である(1230)
  - thread で mutex.lock() する関数を再起呼び出し、相互再帰呼び出しをしないように(1227)
    - `recursive_mutex` を使用し、同一の thread から繰り返し（再帰的に）mutext を獲得するようにする. (`Reentrant mutex`)
  - `mutex エラー`: mutex の処理が失敗することもあり、その場合は、system_error 例外が送出される. (1228)
    - 例えば、lock() をすでに取得しているのに 2 回目の lock() を実行しようとすると、ネイティブがハンドル済みである `device_or_resource_busy` 例外が送出される(1228)
      - これが `lock_guard` や `unique_lock` を利用する理由となる. これらは資源ハンドルで、RALL である. (1230)
      - lock_guard はスコープが外れた時にデストラクタがよばれ、その際にコンストラクタに渡した lock を解放する (1231)
      - unique_lock が保持するロック可能オブジェクトが実際にロックを獲得したかどうかをチェックするには `owns_lock`によって行う(1233)
        - 複数の mutex を複数の thread に送るときは、`unique_lock + defer_lock` と lock(unq_mtx1, unq_mtx2) のように複数 mutex を同時に獲得するような処理を使ってデットロックを防ぐ(1234)
    - lock() は所有権を獲得できない場合は、その時点で thread をブロックする. thread をブロックせずに後続の処理を実行したい場合は、try_lock() を使う.(1229)
    - 再度 lock を試みるために少し時間を置きたい時は、timed_mutex, recursive_timed_muted を利用(1229)
      - これらの処理に渡す duration が 現在時刻の前である場合や、ゼロ以下の場合は try_lock() と同じ
    - mutex などは`ロック可能オブジェクト(lockable object)`と呼ばれる
  - `タスクベースの並列処理`:
    - 代替案、詳細、性能、トレードオフを考えるあまりに、単純であることの価値を見失ってしまうことがある. 基本的にはもっとも単純な技法を使うべきである. 複雑な解は、それが真価を発揮することがわかっている時のみ使うべきである(1241)
    - future と promise が指す value は技術的には`共有状態(shared state)`と呼ばれる(1242)
    - 共有状態の value には次のような情報が含まれる:
      - `適切な型の値(value)、あるいは例外`: void を返す future の場合は値はない.
      - `レディビット(ready bit)`: 値または例外を future から取り出せるかどうかを示す.
      - タスク：deferred 起動ポリシーを伴って async() によって起動されたタスク用の future に対して get() が呼び出された時に実行すべきタスク
      - `参照カウンタ(use count)`: 最後のユーザがアクセスを手放した時のみ、共有状態を解体できるようにする. 内部に保持されている値の型がデストラクタを持つクラスであれば、参照カウンタガゼルになったときに、デストラクタが実行される
      - 何らかの`相互排他データ（mutual exclusion data）`: 待機しているどの thread でもブロック解除できるようにする(例えば、condition_variable)
    - 共有状態では、処理系は次の処理を実行できる:
      - `構築(construct)`: ユーザ指定のアロケーたが利用される可能性がある
      - `レディ化(make ready)`: レディビットをセットし、待機している全ての thread をブロック解除する
      - `中断(abandon)`: promise が値や例外を共有状態に置けなくなった場合に、エラー状態 `broken_promise` の `future_error 例外`を共有状態に置いて、レディ化する
  - promise: `future が値や例外を取り出せるように、タスクが結果を預ける場所である`(1242)
    - set 関数はすでに値や例外がセットされていれば、future_error 例外を送出(1243)
  - future:
    - get() は一回しか呼び出せない(2 回目の get() で future_already_retrieved エラーが発生する). これは、一回呼び出すとその取り出した値を呼び出し先に move するためである(1249)
    - 値を（例えば異なるタスクから）複数回取り出す必要がある場合は、shared_future を利用(1247)
      - shared_future は値を move するのではなく、呼び出し元にコピーする(1249)
      - future.share() で future から shared_future へ move して構築することができる.
      - 返却値が const であれば、複数スレッドから安全に利用できるが、非 const の場合はデータの変更の可能性があるため、何らかの相互排他が必要である.(1250)
  - packaged_task: タスク、future/promise のペアとを保持するものである.
    - pt.get_future() を 2 回呼び出すと future_error になる(1244)
    - pt.make_ready_at_exit(args): f(args) を呼び出す. 実行結果は thread が終了するまで取り出せない.
      - メリットは、thead_local 変数のデストラクタが実行されるまで、結果が利用できないことである.(1245)
    - (デストラクタやムーブによって中断する場合と同様に)共有状態を中断することは、それをレディ化することを意味する. もし値も例外も持っていない場合は、future_error を指すポインタが格納される(1244)
  - `スレッドローンチャ(thread launcher)`: 新しい thread を作成するべきか、それとも、既存の thread を再利用するか、それとも、単に現在のスレッドで実行するのかを判断する関数である(1251)
    - async: 非同期タスクローンチャ. タスクの返却値型が R である時、async() を呼び出すと、future<R> が返却される(1251)
    - ローンチャにはポリシーを渡すことができる:
      - async: あたかも新しい thread がつくられたかのようにタスクを実行
      - deferred: タスクの future に対する get() 時点でタスクを実行
    - スレッドプールは thread を再利用するためにあり、async が繰り返し利用する.(1252)
- `標準 C ライブラリ`:
  - `gets(s)`: とほぼ同等の `scanf("%s", s)` は絶対に使用してはいけない. 入力バッファをオーバーフローさせる入力を与えるなどするとプログラムが破壊されてコンピュータが攻撃者に乗っ取られる可能背がある. (1265)
    - C11 では `gets_s(p, n)`などの引数を追加してオーバーフロー対策を施した, stdio 入力関数群を追加している.
  - getc などの文字処理関数の返却値の型は int である(char だと EOF が返せなくなるため)
  - exit(), abort() がデストラクタを実行しないことに注意. 構築済みオブジェクトに対するデストラクタの実行が必要な場合は、例外を送出する(1271)
  - 型安全と拡張性から `<stdlib>` よりも `<iostream>` を優先する(1271)
  - `malloc()`, `reaaloc()` よりも vector を優先
  - 時間管理は、`<ctime>` よりも `<chrono>` を優先
  - `qsort()` よりも `sort()` を優先
- `互換性`:
  - `禁止事項、非推奨`:
    - 文字列リテラルの `char*` への代入は禁止
    - デストラクタを持つクラスに対するコピーコンストラクタとコピー代入演算の生成は非推奨とされた((85) も合わせて確認.)
  - `非互換`:
    - C 言語で広域 const はデフォルトで外部結合を持つが、C++ では持たない. C++ では明示的に extern しない限り初期化をしないといけない(1282)
    - C 言語ではネストした struct は外側の struct を同じスコープを持つが、C++ ではそうではない.
    - C++ ではクラスめいは宣言した箇所のスコープを持つため、同一スコープないでは同じ名前を別の型で宣言することはできないが、C 言語では可能.
  - 何らかの名前を広域名前空間に置く全ての標準ヘッダ `<X.h>` には、対応する `<cX>` が存在している. これは名前を std 名前空間に置くものである. (1286)
  - C 言語関数を宣言する際は、`extern "C"` を利用
