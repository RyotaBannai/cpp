...
- スレッドとタスク:
  ...
  - `スタベイション(starvation)`: 複数の thread がブロックされている時、ブロック解除すべき thread を選択するするのは、還俗としてシステムスケジューラであり、いつまでも選択されない thread が発生してしまうこと. (1225)
  ...
  - システム資源は、共有、リーク、コピー、ムーブができないので、擬似 t 系に別個のものとして見るのが良い.
  - thread で mutex.lock() する関数を再起呼び出し、相互再帰呼び出しをしないように(1227)
    - recursive_mutex を使用し、同一の thread から繰り返し（再帰的に）mutext を獲得するようにする.
